    ...
    <div>
        ...
        <div class="content">
            ...
            <div class="address clearFix" v-for="item in address" :key="item.id">
                <span class="username" :class="{ selected: item.isDefault === '1' }" @click="toggleAddress(item, address)">
                    {{ item.cosignee }}
                </span>
                <p>
            ...
            <div class="address clearFix" @click="togglePay">
                <span class="username" :class="{ selected: payment === 1 }" :data-payment="1">在线支付</span>
                <span class="username" style="margin-left:5px;" :class="{ selected: payment === 0 }"> 货到付款 </span>
            </div>
            <div div class=" line">
            </div>
            ...
            <div class="detail">
                <h5>商品清单</h5>
                <ul class="list clearFix" v-for="( item, index ) in  orderList " :key="index">
                    <li>
        ...
        <div class="trade">
            ...
            <div class="receiveInfo">
                ...
                收货人：<span>{{ userAddress.cosignee }}</span>
...
export default {
    ...
    computed: {
        ...
        // 该属性是用户最后提交订单时的地址和收货人
        userAddress() {
            let result = this.address.filter(item => item.isDefault === '1')
            return result[0] || {}
        }
    },
    methods: {
        // 切换地址
        toggleAddress(item, itemList) {
            itemList.forEach(item => item.isDefault = '0');
            item.isDefault = '1'
        },
        // 切换支付方式
        togglePay(e) {
        ...
        async submitOrder() {
            ...
            // 要发送给服务器的数据
            let data = {
                consignee: this.userAddress.cosignee,
